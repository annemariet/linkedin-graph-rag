...
def determine_post_type(activity: Dict) -> str:
    ...
    if not is_repost:
        is_repost = bool(activity.get('resharedPost') or activity.get('resharedActivity'))
...
def extract_timestamp(activity: Dict, as_iso: bool = False) -> Optional[any]:
    """
...
def is_post_element(element: Dict) -> bool:
    ...
    """
    resource_name = element.get('resourceName', '')
    return 'ugcPost' in resource_name.lower() or 'ugcPosts' in resource_name
