...
def create_chunk_node(tx, chunk_id: str, text: str, source_urn: str, chunk_index: int, total_chunks: int):
    """
...
def index_content_for_graphrag(driver, embedder, embedding_dimensions: int, limit: Optional[int] = None):
    """
    ...
    for i, node in enumerate(nodes, 1):
        ...
        for chunk_idx, chunk_text in enumerate(chunks):
            ...
            try:
                ...
                print(f"\n💡 Troubleshooting:")
                print(f"   1. Verify the model '{EMBEDDING_MODEL}' is available in your project")
                print(f"   2. Check Google Cloud credentials are properly configured")
                print(f"   3. Try a different model: textembedding-gecko@002 or textembedding-gecko")
        ...
        if chunk_embeddings:
            ...
            with driver.session(database=NEO4J_DATABASE) as session:
                ...
                if verify_result['count'] != len(node_ids):
                    print(f"   ⚠️  Warning: Only {verify_result['count']}/{len(node_ids)} nodes found")
                ...
                if updated_count != len(chunk_ids):
                    raise RuntimeError(f"Only {updated_count}/{len(chunk_ids)} chunks updated")
                ...
                if verify_embeddings != len(node_ids):
                    raise RuntimeError(f"Verification failed: Only {verify_embeddings}/{len(node_ids)} embeddings stored")
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ['-h', '--help']:
            ...
            print("\nOptions:")
            print("  --limit N    Process only the first N posts/comments (for testing)")
    ...
    try:
        ...
        if actual_dimensions != EMBEDDING_DIMENSIONS:
            print(f"   ⚠️  Note: Using actual dimensions ({actual_dimensions}) instead of default ({EMBEDDING_DIMENSIONS})")
    except Exception as e:
        print(f"\n❌ FATAL ERROR: Failed to initialize embedder")
        print(f"   Error: {str(e)}")
        print(f"   Model: {EMBEDDING_MODEL}")
        print(f"\n💡 Troubleshooting:")
        print(f"   1. Verify the model '{EMBEDDING_MODEL}' is available in your Vertex AI project")
        print(f"   2. Check GOOGLE_APPLICATION_CREDENTIALS is set or authentication is configured")
        print(f"   3. Try a different model: textembedding-gecko@002 or textembedding-gecko")
