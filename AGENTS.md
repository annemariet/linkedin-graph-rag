...
- **Commits**: Commit after each small, logical change — not a single batch at the end. One commit = one coherent change (e.g. one fix, one test file, one refactor). Use gitmoji (see https://gitmoji.dev/).
